...
def write_cmd(ROOT, element, cutoff, start, ndim, w):
	...
	w = 0.45 + 0.05 * w
        ...
        else:FILE.write('solvatebox slab {} {{{} {} {}}}\n'.format(element.upper(), size/2.0,size/2.0,size*w/2.0))
...
for T in Trange:
	...
	for cutoff in CUTOFF:
		...
		for w in W_RANGE:
			...
			#if w == 12: write_cube(root, atom, cutoff, T, start, ndim, 2, 16)
			#write_vac(root, atom, cutoff, T, 0, start, ndim, 1, 8)
			write_vac(root, atom, cutoff, T, 3, start, ndim, 1, 16)
			#write_surface(root, atom, cutoff, T, 0, start, ndim, 1, 8)
			write_surface(root, atom, cutoff, T, 3, start, ndim, 2, 16)
