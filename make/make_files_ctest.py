...
def write_cmd(ROOT, element, cutoff, start, ndim, c):
	...
	c = 1 + 0.05 * c
        ...
        else:FILE.write('solvatebox slab {} {{{} {} {}}}\n'.format(element.upper(), size*c/2.0,size*c/2.0,size*c/2.0))
...
for cutoff in CUTOFF:
	ROOT = '{}/CUT_{}_A'.format(ROOT, cutoff)
        if not os.path.exists(ROOT): os.mkdir(ROOT)
        ROOT = '{}/C_TEST'.format(ROOT)
        if not os.path.exists(ROOT): os.mkdir(ROOT)
	for c in C_RANGE:
		root = '{}/C_{}'.format(ROOT, c) 
		if not os.path.exists(root): os.mkdir(root)
		write_cmd(root, atom, cutoff, start, ndim, c)
