...
def R_tensors(directory, frame, nslice, nmol, model, csize, COM, DIM, nsite):
        ...
        with file('{}/DATA/EULER/{}_{}_{}_ODIST.txt'.format(directory, model.lower(), nslice, frame), 'w') as outfile:
...
def mol_angles(directory, frame, nslice, nmol, model, csize, COM, DIM, nsite):
	...
	for j in xrange(nmol):
        ...
        with file('{}/DATA/EULER/{}_{}_ANGLE.txt'.format(directory, model.lower(), frame), 'w') as outfile:
...
def angle_dist(directory, model, csize, nframe, nmol, nsite, COM, DIM, nslice, npi, ow_angles):
        ...
        for frame in xrange(nframe):
                ...
                if ow_angles:
                        angles = mol_angles(directory, frame, nslice, nmol, model, csize, COM, DIM, nsite, nm, n0, phi_)
                        z_array, theta, phi, varphi, zeta_array1, int_theta1, int_phi1, int_varphi1, zeta_array2, int_theta2, int_phi2, int_varphi2 = angles
                else:
                        try:
                                with file('{}/DATA/EULER/{}_{}_ANGLE.txt'.format(directory, model.lower(), frame), 'r') as infile:
	...
	vd = DIM[0] * DIM[1] * DIM[2] / nslice
        ...
        for frame in xrange(nframe):
                ...
                if ow_polar: temp_O = R_tensors(directory, frame, nslice, nmol, model, csize, COM, DIM, nsite)
                        ...
                        try:
                                with file('{}/DATA/EULER/{}_{}_{}_ODIST.txt'.format(directory, model.lower(), nslice, frame), 'r') as infile:
...
def euler_profile(directory, nframe, nslice, nmol, model, csize, suffix, AT, Q, M, LJ, COM, DIM, nsite, a_type, npi, ow_angle, ow_polar):
	...
	with file('{}/DATA/EULER/{}_{}_{}_{}_EUL.txt'.format(directory, model.lower(), a_type, nslice, nframe), 'w') as outfile:
