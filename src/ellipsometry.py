...
def ellips(n, theta, lam_vac, d_list, nslice, pol):

	angles = snell_angle_array(n, theta)
	kz_list = 2 * np.pi * n * np.cos(angles) / lam_vac
	delta = kz_list * d_list

	t_list = np.zeros((nslice/2, nslice/2), dtype=complex)
	r_list = np.zeros((nslice/2, nslice/2), dtype=complex)

	M_list = np.zeros((nslice/2, 2, 2), dtype=complex)
...
def load_theoretical(directory, model, csize, nslice, a_type, DIM, nm, nimage, angles):

	with file('{}/DATA/INTDIELEC/{}_{}_{}_{}_{}_{}_DIE.txt'.format(directory, model.lower(), csize, nslice, a_type, nm, nimage), 'r') as infile:
		int_exx, int_ezz = np.loadtxt(infile)
	with file('{}/DATA/INTDIELEC/{}_{}_{}_{}_{}_{}_CWDIE.txt'.format(directory, model.lower(), csize, nslice, a_type, nm, nimage), 'r') as infile:
		cw_exx_A, cw_ezz_A, cw_exx_B, cw_ezz_B, cw_exx_C, cw_ezz_C = np.loadtxt(infile)
	with file('{}/DATA/ELLIP/{}_{}_{}_{}_{}_{}_ELLIP_NO.txt'.format(directory, model.lower(), csize, nslice, a_type, nm, nimage), 'r') as infile:
		no_sm, cw_no_A, cw_no_B, cw_no_C, cw_no_D = np.loadtxt(infile)

	if os.path.exists('{}/DATA/ELLIP/{}_{}_{}_{}_{}_{}_ELLIP_NE_A.txt'.format(directory, model.lower(), csize, nslice, a_type, nm, nimage)):# and False:
		with file('{}/DATA/ELLIP/{}_{}_{}_{}_{}_{}_ELLIP_NE_A.txt'.format(directory, model.lower(), csize, nslice, a_type, nm, nimage), 'r') as infile:
			cw_ne_A = np.loadtxt(infile)
		with file('{}/DATA/ELLIP/{}_{}_{}_{}_{}_{}_ELLIP_NE_B.txt'.format(directory, model.lower(), csize, nslice, a_type, nm, nimage), 'r') as infile:
			cw_ne_B = np.loadtxt(infile)
		with file('{}/DATA/ELLIP/{}_{}_{}_{}_{}_{}_ELLIP_NE_C.txt'.format(directory, model.lower(), csize, nslice, a_type, nm, nimage), 'r') as infile:
			cw_ne_C = np.loadtxt(infile)
		with file('{}/DATA/ELLIP/{}_{}_{}_{}_{}_{}_ELLIP_NE_D.txt'.format(directory, model.lower(), csize, nslice, a_type, nm, nimage), 'r') as infile:
		...
		centres = np.ones(2) * np.mean(mean_auv1)
		deltas = np.ones(2) * 0.5 * (Delta1 + Delta2)

		nangles = len(angles)

		cw_ne_A = np.zeros((nangles, nslice/2))
		cw_ne_B = np.zeros((nangles, nslice/2))
		cw_ne_C = np.zeros((nangles, nslice/2))
		cw_ne_D = np.zeros((nangles, nslice/2))
		...
		with file('{}/DATA/ELLIP/{}_{}_{}_{}_{}_{}_ELLIP_NE_A.txt'.format(directory, model.lower(), csize, nslice, a_type, nm, nimage), 'w') as outfile:
			np.savetxt(outfile, cw_ne_A, fmt='%-12.6f')
		with file('{}/DATA/ELLIP/{}_{}_{}_{}_{}_{}_ELLIP_NE_B.txt'.format(directory, model.lower(), csize, nslice, a_type, nm, nimage), 'w') as outfile:
			np.savetxt(outfile, cw_ne_B, fmt='%-12.6f')
		with file('{}/DATA/ELLIP/{}_{}_{}_{}_{}_{}_ELLIP_NE_C.txt'.format(directory, model.lower(), csize, nslice, a_type, nm, nimage), 'w') as outfile:
			np.savetxt(outfile, cw_ne_C, fmt='%-12.6f')
		with file('{}/DATA/ELLIP/{}_{}_{}_{}_{}_{}_ELLIP_NE_D.txt'.format(directory, model.lower(), csize, nslice, a_type, nm, nimage), 'w') as outfile:
	...
	no_array = (cw_no_A, cw_no_B, cw_no_C, cw_no_D)
	ne_array = (cw_ne_A, cw_ne_B, cw_ne_C, cw_ne_D)

	return no_array, ne_array

def transfer_matrix(directory, model, csize, AT, sigma, nslice, nimage, a_type, nm, DIM, cutoff):
	...
	with file('{}/DATA/DEN/{}_{}_{}_{}_PAR.txt'.format(directory, model.lower(), csize, nslice, nimage), 'r') as infile:
		param = np.loadtxt(infile)

	Z = np.linspace(0, DIM[2]/2, nslice/2)
	...
	no_array, ne_array = load_theoretical(directory, model, csize, nslice, a_type, DIM, nm, nimage, ellip_angles)

	d_list = np.ones(len(Z)) * lslice
	d_list[0] = np.inf
	d_list[-1] = np.inf
	...
	for i, no in enumerate(no_array):
		...
		for t, theta in enumerate(ellip_angles):
			...
			for p, pol in enumerate(POL):
				...
				r, t = ellips(n, theta, lam_vac, d_list, nslice, pol)
				r_ps.append(r)
		...
		print "surface n = {:.5f}   bulk n = {:.5f}   dn = {:.5f} ({:.2%})".format(np.max(no), no[-1], np.max(no) - n[-1], (np.max(no) - no[-1]) / no[-1])
		print "Ellipsometry direction = {}\n".format(np.sign(Delta[-1] - Delta[0]))


	if model.upper() != 'DMSO': experiment = EXPERIMENT[model.upper()]
	
	plt.figure(0, figsize=(fig_x,fig_y))
        plt.xlabel(r'z Coordinate (\AA)')
        plt.ylabel(r'n (a.u.)')
        plt.legend(loc=2)
        plt.axis([param[3]-10, param[3]+15, 1, 1.5])
        plt.savefig('{}/{}_{}_{}_{}_n_profile.png'.format(groot, model.lower(), nimage,nslice,nm))
	plt.savefig('{}/{}_{}_{}_{}_n_profile.pdf'.format(groot, model.lower(), nimage,nslice,nm))
	...
	plt.legend(loc=4)
	plt.savefig('{}/{}_{}_{}_{}_amplitude.png'.format(groot, model.lower(), nimage,nslice,nm))
	plt.savefig('{}/{}_{}_{}_{}_amplitude.pdf'.format(groot, model.lower(), nimage,nslice,nm))
	...
	plt.legend(loc=2)
	plt.savefig('{}/{}_{}_{}_{}_delta.png'.format(groot, model.lower(), nimage,nslice,nm))
	plt.savefig('{}/{}_{}_{}_{}_delta.pdf'.format(groot, model.lower(), nimage,nslice,nm))
